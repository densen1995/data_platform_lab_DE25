...
- Storage -> Cleaning datasets and saving in a structured format(example PostgreSQL database)
    - *As CSV outputs , in PostgreSQL using Psycopg3 or other tools*
...
**Bonus(Just for testing)**
**Ingestion into a database (PostgreSQL) using PGAdmin**

- Moving data from a file into a database table


    - The data is stored in the database and ready for different analysis after creating a table and importing the csv file transformed using pandas into PostgreSQL(Queryable database).
    - The data can be queried (table analytics_summary)after creating and importing the csv file into PostgreSQL database.
    - Generating a graphical insight of the data/table through a bar chart in pgAdmin:

**Sample Image**

<img src= "analytics_summary.png"  width=500>
